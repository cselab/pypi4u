...
### Structure 
The project is structured into a main directory, a model directory `model_1` and an `engines` directory. The main directory contains the `run.py` script, which on execution either runs the CMA or TMCMC implementation. The `run.py` script accesses the engines folder, which contains the CMA and TMCMC code. The model folder is problem specific, as it contains all of the parameter files and the model function code that define the problem. It is the model folder that user shall alter and modify. 
